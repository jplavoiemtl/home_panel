...
10. **SquareLine Studio files**: Files prefixed with `ui_` (such as `ui.c`, `ui.h`, `ui_Screen*.c`, `ui_helpers.c`, `ui_img_*.c`, etc.) are generated by SquareLine Studio (SLS). Do not modify these files directly as they will be overwritten when the user exports from SLS. The user modifies the UI externally in SquareLine Studio.
